...
- Do not implement health checks, provider/model availability checks, authentication checks, effective configuration precedence, or executable health/config commands; those belong to Sprint 5.
...
- Do not implement persistence hooks, active-run stores, historical inspection, dashboards, cost/time estimation algorithms, or executable status/inspect commands; those belong to Sprint 8 or later.
...
- **Package / Module Boundaries:** Use the Sprint 1 SDK/library package boundary. Keep the public contract in the SDK surface and the fake runtime in the test/fake package or fixture location established by Sprint 1. Do not add executable command behavior.
...
- [x] **Decision 1: Use Runtime/Run/Session/Turn/Event As The Contract Boundary**
...
- [x] **Decision 2: Define Full Lifecycle Vocabulary But Minimal Fake Behavior**
...
- [x] **Decision 3: Use Canonical Event Envelope With Raw Native Payload Preservation**
...
- [x] **Decision 4: Use Open Runtime Identifiers And Capability Discovery**
...
- [x] **Decision 5: Define Typed/Classified SDK Errors Without Policy Execution**
...
- [x] **Decision 6: Prove Contract With Fake Runtime Only**
...
_No user-facing executable surface is in scope for this sprint. Any study references below are internals-only evidence about boundaries, injection, IO, and testability._

- [x] **Task 1: Verify Prerequisites And Existing Boundaries**
  ...
  - [x] **Sub-task 1.1:** Confirm SDK/library package, fake runtime harness, fixture/testdata conventions, and test command from Sprint 1 exist.
  - [x] **Sub-task 1.2:** If Sprint 1 outputs are missing, stop implementation and record the blocker instead of creating broad project skeleton work inside Sprint 2.
  - [x] **Sub-task 1.3:** Confirm `targets/agentwrap/brief.md`, `targets/agentwrap/DECISIONS.md`, this reasoning file, and the evidence bundle are available.

- [x] **Task 2: Define Public Runtime Contract Primitives**
  ...
  - [x] **Sub-task 2.1:** Define stable identifier types for run, session, turn, event, runtime kind, provider, model, and artifact references as needed by the package layout.
  - [x] **Sub-task 2.2:** Define the runtime interface and run/result concepts for starting work, streaming/consuming events, awaiting final result, inspecting metadata, and exposing capabilities.
  - [x] **Sub-task 2.3:** Define request/input shape with prompt, working directory, optional session reference, provider/model preferences, permission/sandbox placeholders, timeout/metadata fields only where needed for the contract.
  - [x] **Sub-task 2.4:** Confirm no task/workflow/DAG/UltraPlan-specific types enter the SDK contract.

- [x] **Task 3: Define Lifecycle, Metadata, Artifacts, And Capabilities**
  > *Description: Make state and runtime support explicit enough for dashboards, auditing, and future adapter work.*
  - [x] **Sub-task 3.1:** Define lifecycle state vocabulary from PRD/TRD and mark states whose behavior is implemented in later sprints.
  - [x] **Sub-task 3.2:** Define run/session/turn metadata fields for runtime kind, provider, model, attempts placeholder, timing, status, warnings/errors, artifacts, usage/cost placeholders, and session retention metadata.
  - [x] **Sub-task 3.3:** Define artifact references as references/metadata, not large content blobs.
  - [x] **Sub-task 3.4:** Define capability discovery for sessions, cancellation, structured events, raw payloads, artifacts, permissions, usage, validation events, and unsupported behavior reporting.

- [x] **Task 4: Define Canonical Event Model**
  > *Description: Add canonical event envelope/categories that product callers can consume without native runtime parsing.*
  - [x] **Sub-task 4.1:** Define event envelope fields for identity, ordering or timestamp, runtime/provider/model context where known, event category/type, payload, and optional raw native payload.
  - [x] **Sub-task 4.2:** Define event categories for lifecycle, session, message/progress, tool, artifact, permission/blocking, usage, warning, recoverable error, fatal error, rate limit, validation, retry/fallback, final result, and unknown/native extension.
  - [x] **Sub-task 4.3:** Define malformed/unknown event representation as explicit error/event paths for fake tests, while deferring real OpenCode recovery policy to Sprint 3.
  - [x] **Sub-task 4.4:** Add documentation/comments that raw native payloads are diagnostic extensions and must be safe to persist or display only after redaction rules are respected.

- [x] **Task 5: Define Typed/Classified Errors**
  > *Description: Make all public runtime failures inspectable by callers and later policies.*
  - [x] **Sub-task 5.1:** Define error categories required by TRD: configuration, health, runtime unavailable, provider unavailable, model unavailable, authentication, permission, rate limit, timeout, cancellation, malformed event, runtime exit, validation, repair exhausted, cleanup, and unknown.
  - [x] **Sub-task 5.2:** Define classification metadata for retryable, fallbackable, user-actionable, unrecoverable, safe detail, operation, and wrapped cause.
  - [x] **Sub-task 5.3:** Add tests for category/classification behavior, wrapping/inspection, redaction-safe detail, and fake runtime error emission.
  - [x] **Sub-task 5.4:** Confirm no public contract path uses panic, string matching, or unclassified errors for expected runtime failures.

- [x] **Task 6: Implement Fake Runtime Contract Proof**
  > *Description: Exercise the public contract with deterministic fake runtime behavior before any real adapter exists.*
  - [x] **Sub-task 6.1:** Make the fake runtime implement the public runtime contract and report deterministic capabilities.
  - [x] **Sub-task 6.2:** Add normal event stream fixture or test sequence: start, lifecycle transitions, message/progress, artifact reference, usage placeholder, final result.
  - [x] **Sub-task 6.3:** Add failure event stream fixture or test sequence: malformed/unknown event, classified runtime error, cancellation state, unsupported capability/session behavior.
  - [x] **Sub-task 6.4:** Add tests that consume events as a caller would, assert final metadata/status, assert raw payload preservation, and assert no OpenCode dependency.

- [x] **Task 7: Review And Decision Log Updates**
  > *Description: Close the sprint with evidence, not just code presence.*
  - [x] **Sub-task 7.1:** Run the Sprint 1/Sprint 2 test command and record results in this tracker.
  - [x] **Sub-task 7.2:** Review public contract names and docs against the reasoning decisions and roadmap quality gate.
  - [x] **Sub-task 7.3:** Update `targets/agentwrap/DECISIONS.md` only for implementation-confirmed accepted decisions, including requirement, evidence, tradeoff, rejected alternative, and risk/follow-up.
  - [x] **Sub-task 7.4:** Record known gaps for Sprint 3 adapter pressure, Sprint 4 lifecycle/cancellation, and Sprint 6 policy behavior.

## Testing And Documentation Checklist

- [x] **Unit Tests:** table-driven tests for identifier validation if present, lifecycle state/result behavior, event envelope construction, capability reporting, and error classification/wrapping.
- [x] **Fixture Tests:** fake runtime event streams for normal, unknown, malformed, partial/failure, final, artifact, raw-payload, and unsupported-capability cases.
- [x] **Integration Tests:** not applicable for real runtime in Sprint 2; any in-process fake integration test is acceptable if it does not launch OpenCode or depend on external binaries.
- [x] **Real Runtime Smoke:** explicitly deferred to Sprint 3 because this sprint is fake-runtime only by roadmap scope.
- [x] **Documentation Updates:** update public SDK comments/docs where the contract introduces reserved states, raw payload safety, capability semantics, and error classification; update `DECISIONS.md` only after implementation evidence exists.

## Risks And Blockers

| Risk | Impact | Mitigation | Status |
| --- | --- | --- | --- |
| Sprint 1 skeleton/fake harness is missing when implementation starts | High | First task verifies prerequisites and stops rather than absorbing Sprint 1 scope | Closed |
| Contract overfits t3code session/thread/turn/item hierarchy | High | Include turn correlation but reject thread/item/workflow concepts in Sprint 2 | Closed |
| Contract underfits real OpenCode events | High | Preserve raw payloads, unknown event path, and require Sprint 3 adapter pressure review | Carried Forward |
| Lifecycle states imply unimplemented retry/fallback/repair behavior | Medium | Document reserved states and test only fake-supported transitions | Closed |
| Error categories become policy execution | Medium | Limit Sprint 2 to classification and wrapping; defer policy decisions to Sprint 6 | Closed |
| Raw payloads leak sensitive runtime/provider data | High | Treat raw payloads as diagnostic data with safe persistence/display rules and redaction follow-up | Carried Forward |
...
- [x] **Success Criteria 1:** Public SDK contract supports fake runtime start, event consumption, final result inspection, and cancellation/error state representation without OpenCode-specific common-path types.
- [x] **Success Criteria 2:** Run/session/turn identifiers, lifecycle states, event envelope/categories, artifact references, metadata, raw payload preservation, capabilities, and classified errors are defined and tested.
- [x] **Success Criteria 3:** Fake runtime implements the contract and deterministic tests prove normal, unknown/malformed, failure, cancellation, unsupported-capability, raw-payload, artifact, and final-result paths.
- [x] **Success Criteria 4:** Every public error surfaced by the contract is classifiable and inspectable without string matching or panic recovery.
- [x] **Success Criteria 5:** No workflow/DAG/task orchestration, real OpenCode adapter, health/config validation, retry/fallback policy, validation/repair implementation, persistence, or user-facing executable surface is added.
- [x] **Success Criteria 6:** Review can plausibly explain how a second runtime would implement the contract through capabilities and canonical events without changing product-level orchestration requirements.

## Study Evaluation

- [x] **Patterns Followed:** runtime-neutral contract, open runtime/provider identifiers, explicit lifecycle state, canonical event envelope with raw payload escape hatch, capability discovery, typed/classified errors, fake-runtime-first tests, table-driven behavior assertions.
- [x] **Anti-Patterns Avoided:** OpenCode generated SDK as public contract, closed runtime enum, raw stdout/log parsing, workflow/DAG abstraction, unclassified string errors, panic for operational failures, real runtime dependency in unit tests, direct terminal/process coupling.
- [x] **Comparison Needed:** Compare completed code and tests against `reasoning.md` Decision Areas 1-6 and evidence dimensions from runtime-contract, session-lifecycle, and testing-strategy packs.
- [x] **Proceed / Iterate:** Proceed to Sprint 3 only if the contract is testable with fake runtime, no foundational public API ambiguity blocks adapter work, and all Sprint 2 non-scope boundaries were respected.
...
- Prerequisite verification: `/home/antonioborgerees/coding/agentwrap` contains root SDK package, private `internal/testkit` fake harness, `testdata/events` fixtures, and existing `go test ./...` command. Sprint 1 tracker references executable files that are not currently present in the implementation repo, but Sprint 2 did not require or recreate user-facing executable behavior.
- Implemented public contract files: `ids.go`, `lifecycle.go`, `runtime.go`, `metadata.go`, `events.go`, and `errors.go`.
- Implemented private fake runtime proof in `internal/testkit/fake_runtime.go` with compile-time `agentwrap.Runtime` conformance.
- Added tests: `errors_test.go`, `lifecycle_test.go`, and `internal/testkit/fake_runtime_test.go`.
- Updated `README.md` scope guardrails for Sprint 2.
- Decision log updated with `DEC-004`, `DEC-005`, and `DEC-006`.
- Verification passed: `GOCACHE=/tmp/agentwrap-go-build go test ./...`.
- Package layout verification passed: `GOCACHE=/tmp/agentwrap-go-build go list ./...`.
- Formatting verification passed: `test -z "$(gofmt -l .)"`.
- Scope review command: `rg "OpenCode|opencode|DAG|workflow|UltraPlan|panic\\(|os\\.Exit|exec\\.Command" -n .` from implementation repo. Findings were limited to README guardrail text; no real runtime invocation, workflow/DAG API, panic path, or process execution was introduced.
- Explicit deferrals: real OpenCode event adapter and malformed native event recovery policy to Sprint 3; cancellation/process cleanup/session lifecycle behavior to Sprint 4; health/config checks to Sprint 5; retry/fallback policy execution to Sprint 6; validation/repair schema behavior to Sprint 7; persistence/dashboard semantics to Sprint 8.
- Post-audit follow-up implemented in `/home/antonioborgerees/coding/agentwrap`: added `CorrelationID` and `CauseEventID` to canonical events, default fake-runtime correlation/causation values, `SDKError.UserDetail`, `SDKError.DebugDetail`, error construction options, and category-based default classification.
- Post-audit verification passed: `GOCACHE=/tmp/agentwrap-go-build go test ./...`, `GOCACHE=/tmp/agentwrap-go-build go list ./...`, and `test -z "$(gofmt -l .)"`.
- Post-audit deferrals retained: first-class `Session` API to Sprint 4, typed event subscription/registry until real event caller pressure exists, and `errors.Is` sentinel support until concrete caller handling patterns justify it.
